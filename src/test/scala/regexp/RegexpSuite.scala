...
trait RegexpSuite[R <: Regexp[R]](
  ...
  val nonEmptyAlphaNumStr =
    Gen
      .posNum[Int]
      .flatMap(n => Gen.stringOfN(n, Gen.alphaNumChar))
  ...
  property(
    "non-empty string matches only given input string"
  ) {
    forAllNoShrink(
      nonEmptyAlphaNumStr,
      nonEmptyAlphaNumStr
  ...
  property("empty does not match any string") {
    ...
    forAllNoShrink(nonEmptyAlphaNumStr) {
      (string: String) =>
        assert(!r.matches(string))
    }
  }
