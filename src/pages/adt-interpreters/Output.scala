enum Output[A]:
  ...
  case FlatMap[A, B](source: Output[A], f: A => Output[B])
      extends Output[B]
  case Value(a: A)
...
val helloHello = hello
  .andThen(Output.print(" ,"))
  .andThen(hello)
  .andThen(Output.print("?"))
...
def oddOrEven(phrase: String): Output[Unit] =
  Output
    ...
    .flatMap(even =>
      if even then
        Output
          .print(s"$phrase has an even number of letters.")
      else
        Output.print(
          s"$phrase has an odd number of letters."
        )
    )

helloHello
  .andThen(Output.print(" "))
  .andThen(oddOrEven("Scala"))
