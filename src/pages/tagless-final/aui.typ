...
You might have noticed that `and` is another name for `product` from `Semigroupal`, which we encountered in @sec:semigroupal. It has exactly the same signature, apart from the name, and it represents the same concept as applied to user interfaces.
