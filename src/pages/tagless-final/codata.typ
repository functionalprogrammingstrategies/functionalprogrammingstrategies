...
This works, but what if we want text that is _both_ red and bold? We cannot express this with our current design, without creating methods for every possible combination of styles. Concretely this means methods like
...
Notice that we have the usual structure for an algebra, which we first met in @sec:interpreters:structure:

+ we have a constructor in `print`;
+ we have two combinators in `printRed` and `printBold`; and
+ we have an interpreter in `run`.
