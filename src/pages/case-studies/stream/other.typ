...
So far we've stuck to the pull-based model, which fell out of the reification strategy. It's natural to wonder what its dual, a *push-based* approach, would look like.
We'll investigate that here, as well as more briefly considering *incremental computing*, a programming model closely related to reactive programming.
...
arbitrary decision to use reification. We might wonder what its dual, using codata,
...
Our starting point is `Emit`, reproduced below.
...
This in turn implies that we must transform our interpreter loop into continuation-passing style.
...
where `cont` is the continuation.
...
We recurse down the `Compiled` data structure, constructing a tree of continuations.
This is equivalent to the recursion we did in the data interpreter,
and the construction of the continuations represents the demand for data.
The principle difference is what happens next:
instead of returning values we have a sequence of calls pushing data up the tree.

Our implementation recreates the continuations on each call to `next`.
It would be more efficient to construct the continuations once.
